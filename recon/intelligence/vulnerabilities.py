...
def cve_candidate_events(
    result: VulnerabilityLookupResult,
    *,
    source_event: Event,
    minimum_nuclei_cpe_score: float = 0.60,
) -> tuple[Event, ...]:
    """Project NVD intelligence into per-target CVE candidate Events.

    The target URL is copied only from the local TECHNOLOGY observation after
    NVD lookup has completed. It is never part of NVD query parameters.
    """

    if source_event.type is not EventType.TECHNOLOGY:
        raise ValueError("CVE candidate projection requires a TECHNOLOGY event")

    target_url = vulnerability_target_url_from_event(source_event)
    ...
    return tuple(
        cve_candidate_event(
            ...
            minimum_nuclei_cpe_score=minimum_nuclei_cpe_score,
        )
        for candidate in result.cves
    )


def cve_candidate_event(
    candidate: CveCandidate,
    *,
    ...
    """Create one explainable unvalidated vulnerability candidate Event."""

    normalized_target = normalize_vulnerability_target_url(target_url)
    ...
    if (
        source_event.scope_state.value == "IN_SCOPE"
        and candidate.cpe_score >= minimum_nuclei_cpe_score
    ):
        tags.add("nuclei-eligible")
    ...
    cvss_severity = (
        candidate.cvss.base_severity
        if candidate.cvss is not None
        else None
    )

    cvss_vector = (
        candidate.cvss.vector
        if candidate.cvss is not None
        else None
    )

    confidence = clamp01(
        source_event.confidence * 0.45
        + candidate.cpe_score * 0.55
    )

    return Event(
        type=EventType.VULNERABILITY_CANDIDATE,
        value=f"{candidate.cve_id}@{normalized_target}",
        source="vulnerabilities:nvd",
        parent_event_id=source_event.event_id,
        scope_state=source_event.scope_state,
        confidence=confidence,
        novelty=max(source_event.novelty, 0.72),
        depth=source_event.depth + 1,
        tags=tags,
        metadata={
            "cve_id": candidate.cve_id,
            "target_url": normalized_target,
            "product": lookup.observation.product,
            "version": lookup.observation.version,
            "vendor_hint": lookup.observation.vendor_hint,
            "matched_cpe": candidate.matched_cpe,
            "cpe_score": candidate.cpe_score,
            "cpe_confidence": next(
                (
                    cpe.confidence
                    for cpe in lookup.cpe_candidates
                    if cpe.cpe_name == candidate.matched_cpe
                ),
                None,
            ),
            ...
            "cwes": list(candidate.cwes),
            "known_exploited": candidate.known_exploited,
            "cisa_exploit_add": (
                candidate.cisa_exploit_add.isoformat()
                if candidate.cisa_exploit_add is not None
                else None
            ),
            "published": (
            ...
            "nvd_vuln_status": candidate.vuln_status,
            "applicability_basis": candidate.applicability_basis,
            "validated_on_target": False,
            "automated_match_on_target": False,
            "manual_validation_required": True,
            "exploitation_attempted": False,
            "credentials_used": False,
            "severity_inference_from_cvss": False,
            "target_identifiers_forwarded_to_nvd": False,
            "nvd_attribution": candidate.nvd_attribution,
        },
    )


def vulnerability_target_url_from_event(event: Event) -> str | None:
    """Return a local HTTP(S) service root associated with a TECHNOLOGY Event."""

    for key in ("observed_on", "url", "target_url"):
        raw = event.metadata.get(key)
        if not isinstance(raw, str) or not raw.strip():
            continue

        try:
            return normalize_vulnerability_target_url(raw)
        except ValueError:
            continue
...
def normalize_vulnerability_target_url(value: str) -> str:
    """Canonicalize a local service root without contacting it."""

    raw = value.strip()
    if not raw:
        raise ValueError("vulnerability target URL must not be blank")

    parsed = urlsplit(raw)
    scheme = parsed.scheme.lower()

    if scheme not in {"http", "https"}:
        raise ValueError("vulnerability target URL must use HTTP or HTTPS")

    if parsed.username is not None or parsed.password is not None:
        raise ValueError("vulnerability target URL must not contain userinfo")

    if parsed.hostname is None:
        raise ValueError("vulnerability target URL requires a hostname")
    ...
    try:
        ...
        raise ValueError("invalid vulnerability target URL port") from exc
