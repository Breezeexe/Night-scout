...
class ActionCapability(StrEnum):
    ...
    AUDITED_TEMPLATE_EXECUTION = "AUDITED_TEMPLATE_EXECUTION"
    ...
    return (
        ...
        ActionDescriptorRule(
            ...
            descriptor=ActionDescriptor(
                ...
                description=(
                    "Local bounded hostname hypothesis generation from "
                    "wordlists and learned naming patterns"
                ),
            ),
        ),
        ActionDescriptorRule(
        ...
        ActionDescriptorRule(
            rule_id="nuclei.validate-cve",
            worker_pattern="nuclei",
            action_pattern="validate_cve",
            descriptor=ActionDescriptor(
                action_class=ActionClass.VULNERABILITY_VALIDATION,
                capabilities=frozenset(
                    {
                        active,
                        read_only,
                        ActionCapability.AUDITED_TEMPLATE_EXECUTION,
                    }
                ),
                description=(
                    "Read-only validation using one locally audited, "
                    "hash-pinned Nuclei HTTP template"
                ),
            ),
            priority=100,
        ),
