...
codex@ams-1-vm-s6b3:~/Night-scout-setup-fix-copies/tests$ cat ../recon/tooling.py
...
def probe_tool(spec: ToolSpec, manifest: ToolsManifest | None = None) -> ToolStatus:
    ...
    version_output = " ".join(
        (version_result.stdout + " " + version_result.stderr).strip().split()
    )
    identity_output = version_output
    identity_ok = version_result.returncode == 0

    if spec.identity_args is not None:
        try:
            identity_result = subprocess.run(
                [resolved, *spec.identity_args],
                capture_output=True,
                text=True,
                timeout=8.0,
                check=False,
            ...
            return ToolStatus(
                ...
                required=required,
                path=resolved,
                version=version_output[:500] or None,
                identity_ok=False,
                detail=f"identity probe failed: {type(exc).__name__}: {exc}",
            )
        identity_output = " ".join(
            (identity_result.stdout + " " + identity_result.stderr).strip().split()
        )
        identity_ok = identity_ok and identity_result.returncode == 0
    ...
    if identity_ok:
        detail = version_output[:500] or f"version probe exit={version_result.returncode}"
    else:
        detail = (
            f"identity verification failed; version={version_output[:240] or '<empty>'}; "
            f"identity={identity_output[:240] or '<empty>'}"
        )
